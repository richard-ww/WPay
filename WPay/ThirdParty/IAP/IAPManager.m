    ...
    if (self.goodsRequestFinished) {
        ...
        if ([SKPaymentQueue canMakePayments]) { //用户允许app内购
            ...
            if (productId.length) {
                ...
                [HUDView showHudWithTitle:[NSString stringWithFormat:@"%@商品正在请求中",productId]];
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    if (product.count == 0) {
        ...
        NSLog(@"无法获取商品信息，请重试");
        [Toast showWithTitle:@"无法获取商品信息，请重试" afterDelay:2];
        ...
        [HUDView showHudWithTitle:[NSString stringWithFormat:@"正在发起购买请求"]];
...
-(void)sendAppStoreRequestBuyPlist:(NSString *)plistPath {
    ...
    [HUDView showHudWithTitle:@"正在验证交易信息"];
...
#warning 在这里将凭证发送给服务器
    ...
    if (self.delegate) {
        [self.delegate successedWithDict:params];
    }
}

/// 验证成功之后 清除凭证
- (void)cleanReceiptData {
...
- (void)showTips:(NSString *)tipStr {
    
//    Toast.show(title: "用户禁止应用内付费购买:"+error)
    [Toast showWithTitle:tipStr afterDelay:2];
}
